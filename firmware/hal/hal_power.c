...
#if CFG_RTC_EN
    final_sleep_time_us += 235;
#else
    final_sleep_time_us += 160;
#endif
    e_tick = hal_sleep_timer_read_tick(SLEEP_TIMER1_ID);
    final_sleep_time_us += hal_sleep_timer_tick_diff(s_tick, e_tick);
    
    us_remaider += final_sleep_time_us%1000;
    if (us_remaider > 1000) {
        us_remaider -= 1000;
        carry = 1;
    }
    
    return (uint32_t )((final_sleep_time_us/1000) + carry);
}
#endif	// CFG_PM_EN
